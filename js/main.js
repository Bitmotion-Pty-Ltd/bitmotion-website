(function($) {
    ...
    function updateScroll() { if ($(window).scrollTop() >= 80) { $(".bit-header").addClass('sticky'); } else { $(".bit-header").removeClass("sticky"); } }
    $(function() {
    ...
    $(window).on("load", function() { $('.app-scbitnss').owlCarousel({ items: 5, nav: false, dots: true, autoplay: true, loop: true, center: false, margin: 30, autoplayTimeout: 3000, autoplayHoverPause: true, autoHeight: true, smartSpeed: 2000, responsive: { 0: { items: 2 }, 520: { items: 3 }, 768: { items: 4 }, 1200: { items: 4 }, 1400: { items: 5 }, 1600: { items: 5 }, } }); });
    $(window).on("load", function() { $('.counter').counterUp({ delay: 10, time: 4500 }); });
    $(window).on("load", function() { $('.video-popup').magnificPopup({ type: 'iframe', mainClass: 'mfp-fade', removalDelay: 160, }); });
